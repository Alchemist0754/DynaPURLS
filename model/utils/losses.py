...
def accuracy(input, target, class_emb, is_test = False, count_by_batch=False):
    ...
    if target != None:
        ...
        if is_test:
            ...
            return cresults, preds, final_scores
        if count_by_batch:
            return (torch.sum(acc_out).item(), len(acc_out)), preds, final_scores
        else:
            return acc, preds, final_scores
    else:
        return None, preds, final_scores